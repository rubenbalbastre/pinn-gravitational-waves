"""
Transfer learning to non-equal non-eccentric systems
https://towardsdatascience.com/transfer-learning-and-twin-network-for-image-classification-using-flux-jl-cbe012ced146
"""
...
model_name = "test/"
test_name = "32neurons/"
output_directory = "../../01_data/02_output/02_transfer_learning/"
...
nn_output = nn_model_case2(model_name, 32, tanh)
_, _, _, _, _, chain_chiphi, chain_pe, _, _ = nn_output

@load "../../01_data/02_output/02_case_2/cost_function/mae/solutions/model_chiphi.bson" chain_phichi
@load "../../01_data/02_output/02_case_2/cost_function/mae/solutions/model_pe.bson" chain_pe

println("Defining new models...")
n_ = 32
transfer_model_chiphi = Chain(
    x -> x,
    Parallel(
        vcat,
        Chain(chain_phichi.layers[1:end]...,),
        Chain(chain_pe.layers[1:2]...,),
        Chain(x -> [x[5], x[6]])
    ),
    Dense(32+2+2, n_, tanh ,init=Flux.zeros32),
    Dense(n_,2, init=Flux.zeros32)
)
transfer_model_pe = Chain(
    x -> x,
    Parallel(
        vcat,
        Chain(chain_pe.layers[1:end]...,),
        Chain(chain_pe.layers[1:2]...,),
        Chain(x -> [x[5], x[6]])
    ),
    Dense(32+2+2, n_, tanh ,init=Flux.zeros32),
...
# non-trainable part
NN_chiphi_params_non_trainable, re_chiphi_non_trainable = Flux.destructure(transfer_model_chiphi[1:2])
NN_chiphi_non_trainable(u, NN_chiphi_params_non_trainable) = re_chiphi_non_trainable(NN_chiphi_params_non_trainable)(u)
# trainable part
NN_chiphi_params, re_chiphi = Flux.destructure(transfer_model_chiphi[3:end])
NN_chiphi(u, NN_chiphi_params) =  re_chiphi(NN_chiphi_params)( re_chiphi_non_trainable(NN_chiphi_params_non_trainable)(u) )

# non-trainable part
NN_pe_params_non_trainable, re_pe_non_trainable = Flux.destructure(transfer_model_pe[1:2])
NN_pe_non_trainable(u, NN_pe_params_non_trainable) = re_pe_non_trainable(NN_pe_params_non_trainable)(u)
# trainable part
NN_pe_params, re_pe = Flux.destructure(transfer_model_pe[3:end])
NN_pe(u, NN_pe_params) =  re_pe(NN_pe_params)( re_pe_non_trainable(NN_pe_params_non_trainable)(u) )
...
plt1 = plot(tsteps_train, exact_train_wf_real, markershape=:none, markeralpha = 0.25, linewidth = 2, alpha = 0.5, label="wform data (Re)", legend_position=:topleft, title= "Train progress "*wave_id_dict[1], titlefontsize = 8, legend_font_pointsize = 6)
plot!(plt1, tsteps_train, pred_waveform_real_train, markershape=:none, markeralpha = 0.25, linewidth = 2, alpha = 0.5, label="wform NN (Re)")
...
const train_losses = []
const train_losses_complete = []
const train_metrics = []
const train_metrics_complete = []
const test_losses = []
const test_metrics = []
...
optimization_increments = [40, 50, 60, 70, 80, 85, 90, 95, 99, 100]
n = length(optimization_increments)
epochs_increments = [100, 100, 100, 100, 100, 100, 100, 100, 100, 100]
...
df_losses = DataFrame(niter=niter, test_name=test_name, train_losses = train_losses_complete, test_losses=test_losses, train_metrics=train_metrics_complete, test_metrics=test_metrics)
...
if save_data
    ...
    Flux.loadparams!(transfer_model_pe[3:end], Flux.params(re_pe(NN_pe_params)))
    Flux.loadparams!(transfer_model_chiphi[3:end], Flux.params(re_chiphi(NN_chiphi_params)))
    @save solutions_dir*"model_chiphi.bson" transfer_model_chiphi
    @save solutions_dir*"model_pe.bson" transfer_model_pe
end
