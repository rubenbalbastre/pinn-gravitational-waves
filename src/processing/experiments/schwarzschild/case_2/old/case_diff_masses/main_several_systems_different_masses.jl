...
for n_neurons in [32]
    ...
    test_name = "test_all_n_different_masses/"
    ...
    for (index, i) in enumerate(optimization_increments)
        ...
        if index < n-3
            ...
            local res = DiffEqFlux.sciml_train(tmp_loss, NN_params,  BFGS(initial_stepnorm=1, linesearch = LineSearches.BackTracking()), cb=callback, maxiters = max_epochs, allow_f_increases=true)
        else
